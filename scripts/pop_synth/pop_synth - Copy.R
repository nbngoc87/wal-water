...
rwt_lt <- glm(rwtbn ~ municd , us,family = "binomial", na.action = na.exclude)
...
sim.f <- function(a) {
  ...
  fam_sim$rwt_prop <- predict(rwt_lt, newdata = fam_sim, type = "response")
...
system.time(testres <- lapply(lvls(hhs_ss_wal$municd), sim.f))
...
head(fitdf[, c("rpage_cat", "pred1", "pred2", "pred1_num", "rpagecat_num", "pred2_num")])
summary(fitdf[, c("obs", "pred1", "pred2")])
anova(tf, tf2)

head(fam_sim)

table(hhs_ss_wal$bltupss)
