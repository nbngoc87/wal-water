...
CEHDads <-
  read.csv(
    here(
      pdir,
      "utilities_survey_Aquawal_CEHD_Wal/surv14_obs_CEHD_nopassword.csv"
    ),
    na.strings = c("", "#NULL!"),
    strip.white = TRUE,
    encoding = "Latin-1"
...
fullads <-
  read.csv(
    here(
      pdir,
      "utilities_survey_Aquawal_CEHD_Wal/addresses/surv14_address_AquaWal.csv"
    ),
    sep = ",",
    na.strings = c("", "#NULL!"),
    strip.white = TRUE,
    encoding = "Latin-1"
  ) # there are duplicated rows in the data >>> check for whether same questionnaire number different address (it's ok to have different questionnaire number for the same address, since it's can be several housholds share a same house or appartement building)
...
AquaWads$munitx <-
  stringi::stri_trans_general(AquaWads$munitx, "Latin-ASCII; upper")
AquaWads$munitx[AquaWads$munitx %in% "ECAUSSINES"] <- "ECAUSSINNES"

AquaWads$local <-
  stringi::stri_trans_general(AquaWads$local, "Latin-ASCII; upper")

AquaWads$street <- trimws(gsub(",.*", "", AquaWads$address))
AquaWads$street <-
  stringi::stri_trans_general(AquaWads$street, "Latin-ASCII; upper")
